...
fetch(`/api/posts/${postId}`)
    ...
    .then(post => {
        ...
        document.getElementById('meta-title').textContent = `${post.title} - Our Journey - I Love Grok`;
        ...
        document.getElementById('meta-og-title').setAttribute('content', `${post.title} - Our Journey - I Love Grok`);
        ...
        document.getElementById('meta-og-url').setAttribute('content', `http://localhost:3000/journey-post.html?id=${post._id}`);
        document.getElementById('meta-og-image').setAttribute('content', post.originalImage ? `/images/${post.originalImage}` : '/images/Journey01_01.jpg');
        ...
        fetch(`/api/posts?category=${post.category}`)
            ...
            .then(posts => {
                ...
                if (currentIndex < posts.length - 1) {
                    ...
                    prevButton.onclick = () => window.location.href = `journey-post.html?id=${prevPost._id}`;
                ...
                if (currentIndex > 0) {
                    ...
                    nextButton.onclick = () => window.location.href = `journey-post.html?id=${nextPost._id}`;
