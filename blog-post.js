...
fetch(`/api/posts/${postId}`)
    ...
    .then(post => {
        ...
        document.getElementById('meta-title').textContent = `${post.title} - Blog - I Love Grok`;
        ...
        document.getElementById('meta-og-title').setAttribute('content', `${post.title} - Blog - I Love Grok`);
        ...
        document.getElementById('meta-og-url').setAttribute('content', `http://localhost:3000/blog-post.html?id=${post._id}`);
        document.getElementById('meta-og-image').setAttribute('content', post.originalImage ? `/images/${post.originalImage}` : '/images/Blog01_01.jpg');
        ...
        fetch(`/api/posts?category=${post.category}`)
            ...
            .then(posts => {
                ...
                if (currentIndex < posts.length - 1) {
                    ...
                    prevButton.onclick = () => window.location.href = `blog-post.html?id=${prevPost._id}`;
                ...
                if (currentIndex > 0) {
                    ...
                    nextButton.onclick = () => window.location.href = `blog-post.html?id=${nextPost._id}`;
